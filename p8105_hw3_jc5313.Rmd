...
data("ny_noaa")
```
Description: This dataset contains `r nrow(ny_noaa)` observations and `r ncol(ny_noaa)` variables. The variables include: weather station id (`id`), date of observation (`date`), precipitation (`prcp`), snowfall (`snow`), snow depth (`snwd`), maximum temperature (`tmax`), and minimum temperature (`tmin`). Missing data seems to be a significant issue, with a lot of missing precipitation (`prcp`) observations in particular.

#### Data cleaning
Next, I am cleaning the data and I will create separate variables for year, month, and day using `separate`. Then, I will use `mutate` to get a 
```{r}
